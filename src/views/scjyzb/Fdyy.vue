	...
	<div style="width: 100%;overflow-x: hidden;padding-left: 15px;padding-right: 15px;">
		...
		<div class="tab-content">
			...
			<div>
				<van-row>
					<van-col span="24">
						<div style="text-align:center;height:220px;width:380px" ref="myChartFdssfhqx"
							id="myChartFdssfhqx"></div>
					</van-col>
				</van-row>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// document.getElementById("myChartFdssfhqx").style.height=height-52+"px";
			this.getToken();
		},
		methods: {
			...
			getSsyxqk: async function() {
				axios.get(`${this.$config.YDMHEJYY}httpRequest/getRequestResult?apiUrl=/gsjs/RealtimemonitoringWind`, {
					params: {},
				}).then((res) => {
					if (res.code == 0) {
						this.fuelSell.sjyxrl = res.data.ratedPower;
						this.fuelSell.sscl = res.data.realPower;
						this.fuelSell.clxs = this.toDecimal(res.data.coefficient);
			...
			getJzyxqk: async function() {
				axios.get(`${this.$config.YDMHEJYY}httpRequest/getRequestResult?apiUrl=/gsjs/OpeMonitoringWind`, {
					params: {},
				}).then((res) => {
					if (res.code == 0) {
			...
			getSsfhqxt: async function() {
				axios.get(`${this.$config.YDMHEJYY}httpRequest/getRequestResult?apiUrl=/gsjs/runningCurveWind`, {
					params: {},
				}).then((res) => {
					if (res.code == 0) {
						var series = [{
						...
						timeData.push(24);
						this.Fdssfhqx(timeData, series);
					}
				});
			},
			//火电负荷曲线
			Fdssfhqx(timeData, series) {
				let myChartLine = echarts.init(this.$refs.myChartFdssfhqx);
				myChartLine.clear();
				var options = {
					title: {
						//text: '火电负荷曲线'
