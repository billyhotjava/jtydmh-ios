<template>
	<div>
		...
		<div class="lytab-wrap">
			<ly-tab :items="tabList" :options="options" @change="handleChange"></ly-tab>
		</div>
		<!-- </div> -->
			<!-- <van-tabs color="#3b87f6" sticky offset-top="70px" @change="handleChange">
				<div v-for="(item,index) in tabList">
					<van-tab :title="item.label" :name="index">
					</van-tab>
				</div>
			</van-tabs> -->
		<div class="background-white tab-content" v-if="dataList != null && dataList.length != 0" style="margin-top: 60px;">

			<van-pull-refresh v-model="isLoading" @refresh="onRefresh">
				<van-list v-model="loading" :finished="finished" @load="onLoad" :offset="10" :immediate-check="false"
					finished-text="没有更多了">
					<div id="list-content" class="bgwhite zx-notice-list bb" v-for="(item,index) in dataList"
						:key="index">
