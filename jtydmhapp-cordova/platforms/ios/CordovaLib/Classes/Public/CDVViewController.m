...
/// 原生收到远程通知后，传递信息给前端 window.messageJump
- (void)didReceiveRemoteNotificationWithInfo:(NSNotification *)notification {
    self.userInfo = [notification.userInfo mutableCopy];
//    NSLog(@"远程通知信息: %@", self.userInfo);
    if (notification.userInfo) {
        NSError *error = nil;
        NSData *data = [NSJSONSerialization dataWithJSONObject:notification.userInfo options:NSJSONWritingPrettyPrinted error:&error];
        NSString *messageAsString = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSString *js = [NSString stringWithFormat:@"javascript:messageJump(%@,%@)", messageAsString, notification.userInfo[@"needGestureVerify"]];
//        NSLog(@"js 字符串: %@", js);
        [((UIWebView *)self.webView) stringByEvaluatingJavaScriptFromString:js];
//        NSLog(@"执行js result: %@", result);
...
/// 原生App切到前台时，传递信息给前端 window.authGest
- (void)applicationDidBecomeActiveShouldGestureVerifyNotification {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (!self.userInfo) {
//            NSLog(@"js 后台进前台 字符串");
            [((UIWebView *)self.webView) stringByEvaluatingJavaScriptFromString:@"javascript:authGest()"];
//            NSLog(@"执行js 后台进前台 result: %@", result);
        } else {
            self.userInfo = nil;
//            NSLog(@"远程通知已跳转");
        }
...
/// 原生收到本地通知（在App内部断网时，进入离线应用时的通知）
- (void)didReceiveLocalNotificationWithInfo:(NSNotification *)notification {
    if (notification.userInfo) {
        NSError *error = nil;
        NSData *data = [NSJSONSerialization dataWithJSONObject:notification.userInfo options:NSJSONWritingPrettyPrinted error:&error];
        NSString *messageAsString = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSString *js = [NSString stringWithFormat:@"javascript:goToOfflineApp(%@)", messageAsString];
//        NSLog(@"js 字符串: %@", js);
        [((UIWebView *)self.webView) stringByEvaluatingJavaScriptFromString:js];
//        NSLog(@"执行js result: %@", result);
    } else {
