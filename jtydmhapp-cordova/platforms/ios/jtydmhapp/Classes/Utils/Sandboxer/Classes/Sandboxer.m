...
- (void)_config {
    ...
    if (!_homeFileURL || _homeFileURL.absoluteString.length == 0) {
...
- (UINavigationController *)homeDirectoryNavigationController {
    if (!_homeDirectoryNavigationController) {
        ...
        directoryContentsTableViewController.modalPresentationStyle = UIModalPresentationFullScreen;
...
- (void)trigger {
    ...
    if (rootViewController.presentedViewController) {
        if (rootViewController.presentedViewController == self.homeDirectoryNavigationController) {
            self.homeDirectoryNavigationController.modalPresentationStyle = UIModalPresentationFullScreen;
            [self.homeDirectoryNavigationController dismissViewControllerAnimated:YES completion:^{
                _homeTitle = @"";
                _homeDirectoryNavigationController = nil;
            }];
        } else {
