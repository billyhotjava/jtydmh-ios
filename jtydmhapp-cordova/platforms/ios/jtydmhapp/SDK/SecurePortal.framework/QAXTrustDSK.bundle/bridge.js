...
(function () {
  console.log("js bridge--------");
  //防止重复注入
  if (window.qaxSDKBridge) {
    return;
  }

  if (!window.onerror) {
    window.onerror = function (msg, url, line) {
      console.log("qaxSDKBridge: ERROR: " + msg + "@" + url + ":" + line);
    };
  }
  console.log("load qaxSDKBridge js...");
  window.qaxSDKBridge = {
    init: init,
    send: send,
  ...
  var QUEUE_HAS_MESSAGE = "__ztnabridge_fetch_queue_messages__";

  //设置默认messageHandler
  function init(messageHandler) {
    if (qaxSDKBridge._messageHandler) {
      //不抛出异常
      console.log("qaxSDKBridge: WARNING: init called twice.");
      return;
    }
    qaxSDKBridge._messageHandler = messageHandler;
    //将native发送在队列的消息分发出去
  ...
  function _doSend(handlerName, data, responseCallback, isResponse) {
    ...
    if (isIOS) {
      ...
      console.log("qaxSDKBridge will call android function: " + handlerName);
      try {
        var fn = eval("window.ztnaandroid." + handlerName);
      } catch (exception) {
        console.log("qaxSDKBridge: ERROR: get handler %o.", exception);
      }
      if (typeof fn === "function") {
        ...
        //调用默认的handler
        window.ztnaandroid.send(handlerName, JSON.stringify(data), callbackId);
      }
    }
  }
  ...
  function _dispatchMessageFromNative(messageJSON) {
    setTimeout(function () {
      ...
      if (message.responseId) {
        ...
        //默认消息handler
        var handler = qaxSDKBridge._messageHandler;
        ...
        try {
          ...
          console.log("qaxSDKBridge: ERROR: javascript handler %o.", exception);
        }
      }
    });
  }
  ...
  //发送bridge完成事件
  var bridge = window.qaxSDKBridge;
  var doc = document;
  var readyEvent = doc.createEvent("Events");
  readyEvent.initEvent("qaxSDKBridgeReady");
