...
typedef void (^pendingLoginHandler)(BOOL result, NSString * _Nullable errorMessage);
...
*/
- (void)runWithParams:(NSDictionary*)params responseHandler:(void (^)( BOOL result, NSString *errorMessage))responseHandler
{
...
//    }];
    ...
    self.secSdk = [SPVPNClient login:params listener:self];
    NSLog(@"%f", SportalSDKVersionNumber);
}
...
//    [[CLProgressHUD sharedInstance] hide];
    ...
    if(self.pendingHandler) {
        self.pendingHandler(NO, [NSString stringWithFormat:@"%lx@%@", errid, errmsg]);
        self.pendingHandler = nil;
    }
