...
{
    _enableShowTakePhoto = YES;
    _enableShowTakeVideo = YES;
    _enableSortAscending = YES;
    _enablePickingVideo = YES;
    _enablePickingImage = YES;
    _enablePickingGif = YES;
    _enablePickingOriginalPhoto = NO;
    _enableShowSheet = NO;
    _enableCrop = NO;
    _enableCircleCrop = NO;
    _enablePickingMuitlpleVideo = YES;
    _videoMaximumDuration = 15;
    _enableSelectedIndex = YES;
...
- (void)getPictures:(CDVInvokedUrlCommand *)command{
    [self.commandDelegate runInBackground:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.enablePickingVideo = [[paramOptions objectForKey:@"enablePickingVideo"] boolValue];
            self.enablePickingImage = [[paramOptions objectForKey:@"enablePickingImage"] boolValue];
            self.videoMaximumDuration = [[paramOptions objectForKey:@"videoMaximumDuration"] integerValue];
        ...
        #pragma mark - 四类个性化设置，这些参数都可以不传，此时会走默认设置
            ...
            imagePickerVc.videoMaximumDuration = self.videoMaximumDuration; // 视频最大拍摄时间
            ...
            [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
                    ...
                    if(isSelectOriginalPhoto) {
                        ...
                        [self saveCompressImage:assets currentIdx:0 compressedPathArray:pathsArr completion:^(NSMutableArray *paths) {
                            ...
                            NSDictionary* result = @{@"result": paths};
                            NSLog(@"文件地址 %@", paths.firstObject[@"url"]);
                            if (paths && paths.count) {
//                                NSDictionary *info = paths.firstObject;
//                                if ([info[@"mediaType"] integerValue] == PHAssetMediaTypeVideo) {
//                                    [self uploadVideoMedia:paths];
//                                    return;
//                                }
                            }
                            CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:result];
    ...
    [self.commandDelegate runInBackground:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (args && args.count > 0) {
                NSString *playerUrl = args.firstObject;
                NSURL *localVideoUrl = [NSURL fileURLWithPath:playerUrl];
                ...
                CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@"开始播放"];
                [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
            } else {
                CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"参数信息不准确"];
                [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
            }
        });
    }];
...
/// 离线数据存入本地
- (void)offlineToLocal:(CDVInvokedUrlCommand *)command {
    [self.commandDelegate runInBackground:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"==== offlineToLocal ====");
            NSArray *args = command.arguments;
            if (args && args.count > 0) {
                NSMutableArray *newImageUrls = [NSMutableArray array];
                NSArray *imageFileList = args.firstObject[@"imageFileList"];
                
                NSMutableArray *newVideoUrls = [NSMutableArray array];
                NSArray *videoFileList = args.firstObject[@"videoFileList"];
                NSString *timeStampString = [self getCurrentTimeIntervalString];
                for (NSDictionary *dict in imageFileList) {
                    NSString *originalUrl = dict[@"url"];
                    if (IsAvailableString(originalUrl)) {
                        NSString *path = [self saveGivenDataToFile:originalUrl timestampString:timeStampString];
                        if (IsAvailableString(path)) {
                            NSMutableArray *tempDict = [dict mutableCopy];
                            [tempDict setValue:path forKey:@"url"];
                            [newImageUrls addObject:tempDict];
                        } else {
                            
//                            CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"离线文件保存失败"];
//                            [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
                        }
                    } else {
//                        CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:[NSString stringWithFormat:@"给定的 %@ 路径不准确", originalUrl]];
//                        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
                    }
                }
                
                for (NSDictionary *dict in videoFileList) {
                    NSString *originalUrl = dict[@"url"];
                    if (IsAvailableString(originalUrl)) {
                        NSString *path = [self saveGivenDataToFile:originalUrl timestampString:timeStampString];
                        if (IsAvailableString(path)) {
                            NSMutableArray *tempDict = [dict mutableCopy];
                            [tempDict setValue:path forKey:@"url"];
                            [newVideoUrls addObject:tempDict];
                        } else {
...
//                        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
                ...
                NSDictionary *newResult = @{
                    @"imageFileList": newImageUrls,
                    @"videoFileList": newVideoUrls
                };
                CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:newResult];
                [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
            } else {
                CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"参数信息不准确"];
                [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
...
/// 上传完离线数据后，删除本地保存的数据
- (void)uploadOfflineDataRemoveLocalData:(CDVInvokedUrlCommand *)command {
    [self.commandDelegate runInBackground:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"==== uploadOfflineDataRemoveLocalData ====");
            NSArray *args = command.arguments;
            if (args && args.count > 0) {
                NSArray *imageFileList = args.firstObject[@"imageFileList"];
                NSArray *videoFileList = args.firstObject[@"videoFileList"];
                NSString *fileTimestampPath = nil;
                for (NSDictionary *dict in imageFileList) {
                    NSString *url = dict[@"url"];
                    if (IsAvailableString(url)) {
                        NSString *lastPath = [url lastPathComponent];
                        NSRange range = [url rangeOfString:lastPath];
                        if (range.location != NSNotFound) {
                            fileTimestampPath = [url substringToIndex:range.location - 1];
                        }
                        NSFileManager *fileManager = [NSFileManager defaultManager];
                        if([fileManager fileExistsAtPath:url]) {
                            BOOL isDelete = [fileManager removeItemAtPath:url error:nil];
                            if (isDelete) {
                                NSLog(@"清空");
                            } else {
                                NSLog(@"删除失败");
                            ...
                            NSLog(@"%@", [NSString stringWithFormat:@"%@ 文件不存在", dict]);
                        }
                    } else {
                        NSLog(@"%@ 路径不存在", url);
                    }
                ...
                BOOL isDelete = [fileManager removeItemAtPath:fileTimestampPath error:nil];
                if (isDelete) {
                    NSLog(@"删除文件夹");
                } else {
                    NSLog(@"删除文件夹失败");
...
- (void)uploadFiles:(CDVInvokedUrlCommand *)command {
    [self.commandDelegate runInBackground:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"==== uploadPictureAndVideo ====");
            NSArray *args = command.arguments;
            if (args && args.count >= 1) {
                dispatch_queue_t dispatchQueue = dispatch_queue_create("com.eccl.jtmh", DISPATCH_QUEUE_CONCURRENT);
                dispatch_group_t group = dispatch_group_create();
                NSDictionary *fileInfo = args.firstObject;
                NSArray *imageInfoList = fileInfo[@"images"];
                NSMutableArray *imageInfoMutableArray = [NSMutableArray array];
                
                NSArray *videoInfoList = fileInfo[@"videos"];
                NSMutableArray *videoInfoMutableArray = [NSMutableArray array];
                dispatch_group_async(group, dispatchQueue, ^{
                    for (int i = 0; i< imageInfoList.count; i++) {
                        ...
                        if (urls && urls.count) {
                            NSString *fileAllName = urls.lastObject;
                            NSLog(@"文件名称 %@", fileAllName);
                            NSData *tempImageData = [NSData data];
                            if ([url hasPrefix:@"file:"]) {
                                tempImageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:url]];
                            } else {
                                tempImageData = [NSData dataWithContentsOfURL:[NSURL fileURLWithPath:url]];
                            }
                            NSLog(@"缓存照片数据1 %@", tempImageData);
                            if (tempImageData == nil) {
                                tempImageData = [NSData dataWithContentsOfFile:url];
                            }
                            NSLog(@"缓存照片数据2 %@", tempImageData);
                            UIImage *image = [UIImage imageWithData:tempImageData];
                            NSLog(@"image %@", image);
                            NSData *imageData = UIImageJPEGRepresentation(image, 0.5);
                            NSLog(@"imageData %lu", imageData.length);
                            if (imageData && imageData.length) {
                                CLUploadFile *uploadFile = [[CLUploadFile alloc] initWithFileData:imageData fileName:fileAllName fileType:CLUploadFileJPEGType];
                                CLUploadInVPNManager *uploadManager = [[CLUploadInVPNManager alloc] init];
                                [uploadManager uploadWithFile:uploadFile completionHandler:^(NSDictionary * _Nonnull result) {
                                    if (result) {
                                        [imageInfoMutableArray addObject:result[@"fileInfo"][@"id"]];
                                        if (imageInfoMutableArray.count == imageInfoList.count) {
                                            NSLog(@"照片第 %d 个。全部完成", i);
                                            NSLog(@"照片上传for循环结束");
                                            NSLog(@"imageInfoMutableArray: %@", imageInfoMutableArray);
                                        } else {
                                            NSLog(@"照片第 %d 个", i);
                                        }
                                    } else {
...
//                                [[CLAPIClient sharedInstance] UPLOAD:file success:^(NSDictionary * _Nullable responseObject) {
//                                    NSLog(@"responseObject: %@", responseObject);
//                                    [imageInfoMutableArray addObject:responseObject[@"fileInfo"][@"id"]];
//                                    if (imageInfoMutableArray.count == imageInfoList.count) {
//                                        NSLog(@"照片第 %d 个。全部完成", i);
//                                        NSLog(@"照片上传for循环结束");
//                                        NSLog(@"imageInfoMutableArray: %@", imageInfoMutableArray);
...
//                                }];
                ...
                dispatch_group_async(group, dispatchQueue, ^{
                    for (int i = 0; i< videoInfoList.count; i++) {
                        ...
                        NSLog(@"进入视频循环");
                        NSLog(@"视频 enter");
                        NSDictionary *videoInfo = videoInfoList[i];
                        NSString *fileType = nil;
                        NSString *url = videoInfo[@"url"];
                        NSArray *urls = [url componentsSeparatedByString:@"/"];
                        if (urls && urls.count) {
                            NSString *fileAllName = urls.lastObject;
                            NSArray *videoInfos = [fileAllName componentsSeparatedByString:@"."];
                            if (videoInfos && videoInfos.count >= 2) {
                                NSString *videoType = videoInfos.lastObject;
                                if ([CLUploadFileMP4Type containsString:videoType]) {
                                    fileType = CLUploadFileMP4Type;
                                } else {
                                    fileType = CLUploadFileMOVType;
                                }
                                NSData *videoData = [NSData dataWithContentsOfURL:[NSURL fileURLWithPath:url]];
                                if (videoData && videoData.length) {
                                    CLUploadFile *uploadFile = [[CLUploadFile alloc] initWithFileData:videoData        fileName:fileAllName fileType:fileType];
                                    CLUploadInVPNManager *uploadManager = [[CLUploadInVPNManager alloc] init];
                                    [uploadManager uploadWithFile:uploadFile completionHandler:^(NSDictionary * _Nonnull result) {
                                        if (result) {
                                            NSLog(@"上传 result: %@", result);
                                            [videoInfoMutableArray addObject:result[@"fileInfo"][@"id"]];
                                            if (videoInfoMutableArray.count == videoInfoList.count) {
                                                NSLog(@"视频第 %d 个。全部完成", i);
                                                NSLog(@"视频上传for循环结束");
                                                NSLog(@"videoInfoMutableArray: %@", videoInfoMutableArray);
                                            } else {
                                                NSLog(@"视频第 %d 个", i);
                                            }
                                        } else {
...
//                                    [[CLAPIClient sharedInstance] UPLOAD:file success:^(NSDictionary * _Nullable responseObject) {
//                                        NSLog(@"responseObject: %@", responseObject);
//                                        [videoInfoMutableArray addObject:responseObject[@"fileInfo"][@"id"]];
//                                        if (videoInfoMutableArray.count == videoInfoList.count) {
//                                            NSLog(@"视频第 %d 个。全部完成", i);
//                                            NSLog(@"视频上传for循环结束");
//                                            NSLog(@"videoInfoMutableArray: %@", videoInfoMutableArray);
...
//                                    }];
                ...
                dispatch_group_notify(group, dispatch_get_main_queue(), ^{
                    NSLog(@">>>>>>>>>> upload over.");
                    NSString *photoId = @"";
                    if (imageInfoMutableArray.count) {
                        photoId = [imageInfoMutableArray componentsJoinedByString:@","];
                    }
                    NSString *videoId = @"";
                    if (videoInfoMutableArray.count) {
                        videoId = [videoInfoMutableArray componentsJoinedByString:@","];
                    }
                    NSDictionary* result = @{@"photoId": photoId, @"videoId": videoId};
                    CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:result];
                    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
            //        _isSelectOriginalPhoto = FALSE;
                });
...
//                    UIImage *newImage   = [UIImage imageWithData:compressed];
                    ...
                    NSString *fileName = [[[originName lastPathComponent] stringByDeletingPathExtension] stringByAppendingPathExtension:originExt];
                    NSString *path = [self saveNSDataToFile:compressed withName:fileName];
//                    NSString *base64Data = [compressed base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
                    NSDictionary* fileObj = @{
...
//                                              @"size": @([compressed length])
            ...
            [[TZImageManager manager] getVideoOutputPathWithAsset:asset success:^(NSString *outputPath) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [compressedPaths addObject:fileInfo];
                    index += 1;
                    [self saveCompressImage:assets currentIdx:index compressedPathArray:compressedPaths completion:completion];
                });
            } failure:^(NSString *errorMessage, NSError *error) {
                NSLog(@"video errorMessage: %@", errorMessage);
            }];
        }
    }
    else {
...
//    NSString *storeDir = [NSDocumentDirectory() stringByAppendingPathComponent:@"Offline Files"];
...
//保存给定的文件url到Documents/OfflineFiles 目录下
- (NSString *)saveGivenDataToFile:(NSString *)orginalUrl timestampString:(NSString *)timestampString {
//    NSData *fileData = [NSData dataWithContentsOfFile:orginalUrl];
    NSData *tempImageData = [NSData data];
    if ([orginalUrl hasPrefix:@"file:"]) {
        tempImageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:orginalUrl]];
    } else {
        tempImageData = [NSData dataWithContentsOfURL:[NSURL fileURLWithPath:orginalUrl]];
    }
    NSLog(@"缓存照片数据11 %@", tempImageData);
    if (tempImageData == nil) {
        tempImageData = [NSData dataWithContentsOfFile:orginalUrl];
    }
    NSLog(@"缓存照片数据22 %@", tempImageData);
    if (tempImageData && tempImageData.length) {
        NSString *fileName = [orginalUrl lastPathComponent];
        if (IsAvailableString(fileName)) {
            // 获取沙盒临时路径
            NSString *path = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
            NSString *storeDir = [[path stringByAppendingPathComponent:@"OfflineFiles"] stringByAppendingPathComponent:timestampString];
            BOOL isDir = YES;
            BOOL exists = [[NSFileManager defaultManager] fileExistsAtPath:storeDir isDirectory:&isDir];
            if(!exists) {
                [[NSFileManager defaultManager] createDirectoryAtPath:storeDir withIntermediateDirectories:YES attributes:nil error:nil];
            }
            NSString *fullPath = [storeDir stringByAppendingPathComponent:fileName];
            // 将图片写入文件
            BOOL result = [tempImageData writeToFile:fullPath atomically:NO];
            if (result) {
                return fullPath;
            } else {
                NSLog(@"保存本地时，照片写入文件失败");
                return nil;
            }
        } else {
            NSLog(@"保存本地时，照片名称为空");
...
- (void)printAssetsName:(NSArray *)assets {
    ...
    for (id asset in assets) {
        if ([asset isKindOfClass:[PHAsset class]]) {
            PHAsset *phAsset = (PHAsset *)asset;
            fileName = [phAsset valueForKey:@"filename"];
            NSLog(@"选中的资源是: %ld", (long)phAsset.mediaType);
        } else if ([asset isKindOfClass:[ALAsset class]]) {
            ALAsset *alAsset = (ALAsset *)asset;
            fileName = alAsset.defaultRepresentation.filename;;
        }
        NSLog(@"选中的资源名字: %@",fileName);
    }
}
