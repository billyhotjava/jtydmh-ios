...
//NOTE: HarmonyCloud features
#if TARGET_OS_IOS
extern NSString* const Messaging;
extern NSString* const CrashReporting;
extern NSString* const AutoViewTracking;
#endif
extern NSString* const APM;
...
@interface HarmonyCloudConfig : NSObject

/**
 * County Server's URL without the slash at the end.
 * @discussion e.g. https://myHarmonyCloudserver.com
 */
@property (nonatomic, strong) NSString* host;


@property (nonatomic, strong) NSString* screenRecordHost;

/**
 * Application's App Key found on HarmonyCloud Server's "Management > Applications" section.
 * @discussion Using API Key or App ID will not work.
 */
@property (nonatomic, strong) NSString* appKey;

/**
 * For specifiying which features HarmonyCloud will start with.
 * @discussion Available features:
 * @discussion @c Messaging for push notifications,
 * @discussion @c CrashReporting for crash reporting,
 * @discussion @c AutoViewTracking for auto view tracking and
 * @discussion @c APM for application performance management.
 */
 ...
 * @discussion Test push notifications can be to test devices by checking "Send to test device" checkbox on "Create Message" section on HarmonyCloud Server.
 */
...
/**
 * For forcing device ID initialization on start. When it is set, persistenly stored device ID will be reset and new device ID will be re-initialized with @c deviceID property on @c HarmonyCloudConfig object.
 */
 ...
 * @discussion If set, every request sent to HarmonyCloud server will have this custom HTTP header and its value will be @c customHeaderFieldValue property. If @c customHeaderFieldValue is not set when HarmonyCloud is started, requests will not start until it is set using @c setCustomHeaderFieldValue: method later.
 */
 ...
 * @discussion If set, every request sent to HarmonyCloud server will have @c checksum value generated by SHA1(request + secretSalt)
 */
