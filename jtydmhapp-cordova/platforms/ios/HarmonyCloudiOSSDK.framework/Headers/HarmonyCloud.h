...
/**
 *  Represents the APM core class.
 *
 *  @version 1.0
 */
@interface HarmonyCloud : NSObject

#pragma mark - HarmonyCloud Core

/**
 * Returns @c HarmonyCloud singleton to be used throughout the app.
 *
 * @return The shared @c HarmonyCloud object
 */
+ (instancetype)sharedInstance;

/**
 * Starts HarmonyCloud with given configuration and begins session.
 *
 * @param config @c HarmonyCloudConfig object that defines host, app key, optional features and other settings
 */
- (void)startWithConfig:(HarmonyCloudConfig *)config;
// - (void)startWithConfig:(HarmonyCloudConfig *)config window:(UIWindow *)window;

// - (void)startRecordingWithConfig:(HarmonyCloudConfig *)config window:(UIWindow *)window;
...
/**
 * Suspends HarmonyCloud, add recorded events to request queue and ends current session.
 *
...
/**
 * Resumes HarmonyCloud, begins a new session after app comes to foreground.
 *
 * @discussion This method needs to be called manually only on @c watchOS, on other platforms it will be called automatically.
 */
- (void)resume;
...
#pragma mark - HarmonyCloud CustomEvents
...
#pragma mark - HarmonyCloud Messaging
#if TARGET_OS_IOS
/**
 ...
 *
 * @return the mutable set for notification categories.
 */
- (NSMutableSet *) harmonyCloudNotificationCategories;
 ...
 * @return the mutable set for notification categories.
 */
- (NSMutableSet *) harmonyCloudNotificationCategoriesWithActionTitles:(NSArray *)actions;

/**
 * Method that does automatic processing for HarmonyCloud Messaging:
 * - It records that the message has been received.
 * - In case of standard HarmonyCloud messages (Message, URL, Update, Review) it displays alert with app name as a title:
 ...
 * @return TRUE When HarmonyCloud has successfully handled notification and presented alert, FALSE otherwise.
 */
...
/**
 * Method records push opened event. Quite handy if you do not call handleRemoteNotification: method, but still want to see conversions in HarmonyCloud Dashboard.
 *
 * @param c NSDictionary of @"c" userInfo key.
 */
- (void)recordPushOpenForHarmonyCloudDictionary:(NSDictionary *)c;

/**
 * Method records push action event. Quite handy if you do not call handleRemoteNotification: method, but still want to see conversions in HarmonyCloud Dashboard.
 *
 * @param c NSDictionary of @"c" userInfo key.
 */
- (void)recordPushActionForHarmonyCloudDictionary:(NSDictionary *)c;
...
#pragma mark - HarmonyCloud CrashReporting
#if TARGET_OS_IOS
/**
...
#pragma mark - HarmonyCloud AutoViewTracking
...
#pragma mark - HarmonyCloud StarRating
#if TARGET_OS_IOS
/**
