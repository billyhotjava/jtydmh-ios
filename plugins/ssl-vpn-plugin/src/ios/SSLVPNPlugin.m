    ...
    [self.commandDelegate runInBackground:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (args && args.count == 5) {
                ...
                [[SDKWrapper getInstance] runWithParams:param responseHandler:^(BOOL result) {
                    if(result) {
                        NSLog(@"VPN Login Succees");
                        CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@"vpn login success."];
                        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
                    } else {
                        NSLog(@"VPN Login Fail");
                        CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"vpn login failed"];
                        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
                    }
                }];
            } else {
